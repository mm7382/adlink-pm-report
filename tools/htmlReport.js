...
function getOutputHtmlPath(outputDir = OUTPUT_DIR) {
  return path.join(outputDir, `critical_issues_report_${getTaipeiDateStamp()}.html`);
}
...
function generateCriticalIssuesHtml(results, options = {}) {
  ...
  const items = Array.isArray(results) ? results.map(normalizeResultItem) : [];
...
<head>
  <meta charset="UTF-8" />
  <title>ADLINK PM Agent - Critical Issues Report</title>
  <style>
    * {
...
<body>
  <main class="container">
    <header class="header">
      ...
      <h1>Critical xxx open Issues</h1>
...
module.exports = { generateCriticalIssuesHtml };
